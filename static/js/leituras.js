    ...
    async function carregarLivros() {
        try {
            ...
            livros.forEach(livro => {
                const imagem = livro.imagem && livro.imagem.trim() !== ""
                    ? livro.imagem
    ...
    formLeitura.addEventListener("submit", async (e) => {
        ...
        try {
            ...
            if (resp.ok) {
                ...
                let erro;
                try { erro = await resp.json(); }
                catch { erro = { erro: await resp.text() }; }
...
//document.addEventListener("DOMContentLoaded", iniciarMinhasLeituras);
