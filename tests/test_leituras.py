...
def test_registrar_leitura_sucesso(client):
    ...
    with patch("controllers.auth_utils.bd.usuarios", {"123": mock_usuario}), patch(
        "controllers.auth_utils.bd.livros", {"456": mock_livro}
    ), patch(
        "controllers.leituras_controller.bd.cadastrarLeitura", return_value=mock_leitura
    ):
