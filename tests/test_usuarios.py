...
def test_cadastro_usuario_sucesso(client):
    with patch("controllers.usuarios_controller.bd.cadastrarUsuario") as mock_cadastrar:
        mock_cadastrar.return_value = type(
            "Usuario",
            (),
            {
                "to_dict": lambda self: {
...
def test_cadastro_usuario_email_existente(client):
    with patch(
        "controllers.usuarios_controller.bd.cadastrarUsuario", return_value=None
    ):
        response = client.post(
            "/usuarios/cadastro",
            json={
...
def test_login_usuario_sucesso(client):
    ...
    with patch(
        "controllers.usuarios_controller.bd.buscarEmail", return_value=mock_usuario
    ):
        response = client.post(
            "/usuarios/login", json={"email": "teste@teste.com", "senha": "123456"}
        )
